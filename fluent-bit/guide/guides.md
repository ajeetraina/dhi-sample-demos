...
The image's entrypoint is the `fluent-bit` binary. Unlike some Fluent Bit distributions, the hardened image does **not**
auto-load a configuration file at startup — you must pass a config explicitly with the `-c` flag. A reference
configuration (`fluent-bit.conf`) and parser files are bundled read-only at `/fluent-bit/etc/conf/` inside the image for
you to copy or override.

Mount your own configuration at `/fluent-bit/etc/conf/fluent-bit.conf` and pass `-c` so Fluent Bit loads it:

```bash
$ docker run -d --name fluent-bit -p 24224:24224 -p 9880:9880 -p 2020:2020 \
...
and metrics). Note that port `2020` only listens when `http_server On` is set in the `[SERVICE]` block of your config —
the port is exposed in image metadata but nothing binds to it by default.
