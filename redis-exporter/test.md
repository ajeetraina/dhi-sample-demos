...
Run the following command and replace `dockerdevrel` with your organization's namespace and `1.80.1` with the image variant you want to run.

**Available tags for `dockerdevrel/dhi-redis-exporter`:**
...
**Available tags for `dockerdevrel/dhi-redis`:**
...
services:
  redis:
    image: dockerdevrel/dhi-redis:1.80.1
...
$ docker run --rm -it --pid container:my-container \
  ...
  dockerdevrel/dhi-redis-exporter:1.80.1 /dbg/bin/sh
```
...
- Use `dockerdevrel/dhi-redis-exporter:1.80.1` for standard production deployments
- Use `dockerdevrel/dhi-redis-exporter:1.80.1-fips` for FIPS-compliant environments
...
$ docker inspect dockerdevrel/dhi-redis-exporter:1.80.1 | grep fips
