...
To scrape a single Redis instance:

```
$ docker run -d \
  --name redis-exporter \
  -p 9121:9121 \
  <your-namespace>/redis-exporter:<tag> \
  --redis.addr=redis://redis-server:6379
```

This starts the exporter and exposes metrics from the Redis instance at `http://localhost:9121/metrics`.

To configure Redis Exporter using environment variables instead of flags:
...
To run in multi-target mode using the `/scrape` endpoint, start the exporter with an empty `--redis.addr` value:
...
Then scrape metrics from different Redis instances like this:

```
http://localhost:9121/scrape?target=redis://redis-server-1:6379
http://localhost:9121/scrape?target=redis://redis-server-2:6380
```

To expose metrics on a custom port or path, or apply a custom namespace:
...
To export specific keys and their values:
...
The `/health` endpoint is also available for basic health checks:
