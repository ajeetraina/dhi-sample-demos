---
title: Scan an image
description: Learn how to scan Docker Hardened Images using Docker Scout, Grype, Trivy, and Wiz
keywords: dhi, scan, vulnerability, docker scout, grype, trivy, wiz, vex
---
...
For more detailed filtering and JSON output, see
[Docker Scout CLI reference](/reference/cli/docker/scout/).
...
you must build with `--provenance=max` so that Docker Scout can trace the base
image lineage and correctly apply VEX statements.

Without `--provenance=max`, Docker Scout cannot identify the DHI base image in
...
To build with maximum provenance, use the `--provenance=max` flag with
...
> **Important**
>
> The `--provenance=max` flag requires BuildKit with the `docker-container` or
...
After building with `--provenance=max`, Docker Scout reads the full provenance
...
The following is a sample GitHub Actions workflow that builds an image with
`provenance: max` and scans it using Docker Scout:
    ...
    steps:
      ...
      - name: Build and push Docker image
        ...
        with:
          ...
          provenance: max   # Required: enables Docker Scout to trace the DHI base image and apply VEX statements
...
> **Note**
>
> When using `docker/build-push-action`, set `provenance: max` in the action's
> `with` block. This passes `--provenance=max` to BuildKit automatically — no
> additional CLI flags are required.
