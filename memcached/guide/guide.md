...
Run the following command to run a memcached container. Replace `<your-namespace>`
with your organization's namespace and `<tag>` with the image variant you want to run.

```
$ docker run --rm -p 11211:11211 <your-namespace>/dhi-memcached:<tag>
```

### Configuration

To customize the configuration of the memcached server, first obtain the
upstream configuration options from the container. Replace `<your-namespace>` with
your organization's namespace and `<tag>` with the image variant you want to run.

```
$ docker run --rm <your-namespace>/dhi-memcached:<tag> -h
```

Then, run the container with your configuration options.

```
$ docker run --name my-memcache -p 11211:11211 -d <your-namespace>/dhi-memcached:<tag> --memory-limit=64
```
