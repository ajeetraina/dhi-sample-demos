...
Assuming that `config.alloy` file is available in your current directory that contains the configuration for Alloy, you can run the following command to start the container with a bind mount to the `config.alloy` file and a named volume for Alloy's data.
