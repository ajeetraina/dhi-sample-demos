...
- Public image: `dockerdevrel/dhi-alloy:<tag>`
- Mirrored image: `<your-namespace>/dhi-alloy:<tag>`

The public image is available on Docker Hub and does not require authentication.

## Quick start

```bash
# Pull the public image (no auth needed!)
docker pull dockerdevrel/dhi-alloy:1.12.1-debian13
...
docker run --rm -d \
  ...
  -p 12345:12345 \
  dockerdevrel/dhi-alloy:1.12.1-debian13 \
...
# Cleanup
docker stop alloy-quick-test
docker volume rm alloy-data
```

## Start a Grafana Alloy container

The following example assumes there is a file named `config.alloy` in your current directory that contains the configuration for Alloy.

You can run the following command to start the container with a bind mount to the `config.alloy` file and a named volume for Alloy's data.
