...
- **Core (default) variant**: A minimal runtime image with the StatsD Exporter binary, essential runtime dependencies, no shell or package management tools, and runs as nonroot user.
- **Dev variant (`-dev`)**: A build-time image with package management tools, shell access (bash/sh), and runs as root user for configuration and customization.
...
docker run -d \
  ...
  -p 9102:9102 \
  -p 9125:9125/udp \
  dhi.io/prometheus-statsd-exporter:0.28.0
```

This starts the exporter to listen for StatsD metrics on UDP port 9125 and expose Prometheus metrics at `http://localhost:9102/metrics`.
...
services:
  statsd-exporter:
    image: dhi.io/prometheus-statsd-exporter:0.28.0
    ports:
      - "9102:9102"
      - "9125:9125/udp"
    volumes:
...
> **Note:** The DHI Prometheus image runs as nonroot (UID 65532). Using `tmpfs` with the correct uid/gid ensures the container can write to the data directory. For persistent storage, create a volume with appropriate ownership.
...
statsd-dhi-test-prometheus-1        dhi.io/prometheus:3.9                      "prometheus --config…"   prometheus        6 seconds ago   Up 5 seconds   0.0.0.0:9090->9090/tcp
statsd-dhi-test-statsd-exporter-1   dhi.io/prometheus-statsd-exporter:0.28.0   "/usr/local/bin/stat…"   statsd-exporter   6 seconds ago   Up 5 seconds   0.0.0.0:9102->9102/tcp, 0.0.0.0:9125->9125/udp
```
...
To use the StatsD Exporter hardened image in Kubernetes, [set up authentication](https://docs.docker.com/dhi/how-to/k8s/)
...
```console
$ docker run -d --name statsd-test dhi.io/prometheus-statsd-exporter:0.28.0
...
dhi.io/prometheus-statsd-exporter   0.28.0        4.96 MB
dhi.io/prometheus-statsd-exporter   0.28.0-fips   16.75 MB

# Verify FIPS compliance using image labels
$ docker inspect dhi.io/prometheus-statsd-exporter:0.28.0-fips \
