...
### Use case 3: MongoDB replica set for high availability

Deploy a MongoDB replica set using Docker Compose for production-grade deployments.
...
services:
  mongo1:
    image: dockerdevrel/dhi-mongodb:8.0.15
...
After starting the containers, initialize the replica set:
...
### Use case 5: MongoDB backup and restore

Automate MongoDB backups using Docker volumes and mongodump.
...
docker run --rm \
  ...
  dockerdevrel/dhi-mongodb:8.0.15-dev \
  sh -c "mongodump --host localhost --port 27017 \
...
docker run --rm \
  --network container:mongodb \
  -v $(pwd)/backups/mongodb-backup-20250127-120000:/backup \
  dockerdevrel/dhi-mongodb:8.0.15-dev \
  sh -c "mongorestore --host localhost --port 27017 \
    --username admin --password secure_password \
    --authenticationDatabase admin \
    /backup"
```

### Use case 6: MongoDB monitoring and health checks

Implement health checks to monitor MongoDB availability.
...
docker inspect --format='{{range .State.Health.Log}}{{.Output}}{{end}}' mongodb
```
